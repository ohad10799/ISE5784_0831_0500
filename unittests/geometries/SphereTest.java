...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        final var exp_200 = List.of(p200);
        ...
        assertEquals(List.of(p000, p200), result2, "Ray crosses sphere");
        ...
        assertEquals(exp_200, result3, "Ray starts inside the sphere");
        ...
        assertEquals(exp_200, result11, "Ray starts at sphere and goes inside");
        ...
        var exp13 = List.of(p200, p000);
        var result13 = sphere.findIntersections(new Ray(new Point(3, 0, 0), v100opps)).stream().sorted(Comparator.comparingDouble(p -> p.distance(new Point(3, 0, 0)))).toList();
        assertEquals(2, result13.size(), "Wrong number of points");
        assertEquals(exp13, result13, "Ray starts before the sphere");

        // TC14: Ray starts at sphere and goes inside (1 point)
        var result14 = sphere.findIntersections(new Ray(p000, v100));
        assertEquals(1, result14.size(), "Wrong number of points");
        assertEquals(List.of(p200), result14, "Ray starts at sphere and goes inside");
        ...
        assertEquals(exp_200, result16, "Ray starts at the center");

        // TC17: Ray starts at sphere and goes outside (0 points)
        assertNull(sphere.findIntersections(new Ray(p200, v100)), "Ray starts at sphere and goes outside");

        // TC18: Ray starts after sphere (0 points)
        assertNull(sphere.findIntersections(new Ray(pAfter, v110)), "Ray starts after sphere");
        ...
        assertNull(sphere.findIntersections(new Ray(p200, new Vector(0, 1, 0))), "Ray starts at the tangent point");

        // TC21: Ray starts after the tangent point
        assertNull(sphere.findIntersections(new Ray(new Point(2, 1, 0), new Vector(0, 1, 0))), "Ray starts after the tangent point");

        // **** Group: Special cases
        // TC22: Ray's line is outside, ray is orthogonal to ray start to sphere's center line
        assertNull(sphere.findIntersections(new Ray(p01, new Vector(0, 1, 0))), "Ray's line is outside, ray is orthogonal to ray start to sphere's center line");
        // TC23: Ray's line is inside, ray is orthogonal to ray start to sphere's center line
        var result23 = sphere.findIntersections(new Ray(pInside, new Vector(0, 1, 0)));
        assertEquals(1, result23.size(), "Wrong number of points");
        assertEquals(List.of(new Point(0.5, 0.8660254037844386, 0.0)), result23, "Ray starts at the center");
    }
