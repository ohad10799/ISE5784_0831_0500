...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        final Point pInside = new Point(0.5, 0.5, 0);
        ...
        var result1=sphere.findIntersections(new Ray(p01,v110)).stream().sorted(Comparator.comparingDouble(p->p.distance(p01))).toList();
        assertEquals(2, result1.size(), "Wrong number of points");
        assertEquals(exp, result1, "Ray crosses sphere");

        // TC03: Ray starts inside the sphere (1 point)
        var result2 = sphere.findIntersections(new Ray(pInside, v110)).stream().sorted(Comparator.comparingDouble(p -> p.distance(pInside))).toList();
        assertEquals(1, result2.size(), "Wrong number of points");
        assertEquals(exp2, result2, "Ray starts inside the sphere");
        ...
        var result11 = sphere.findIntersections(new Ray(p11, v110)).stream().sorted(Comparator.comparingDouble(p -> p.distance(p11))).toList();
        ...
        var result12 = sphere.findIntersections(new Ray(pInside, v310)).stream().sorted(Comparator.comparingDouble(p -> p.distance(pInside))).toList();
        ...
        Point p13 = new Point(-1, -1, 0);
        Point gp13_1 = new Point(0.5, 0.5, 0.5);
        Point gp13_2 = new Point(1, 1, 1);
        var exp13 = List.of(gp13_1, gp13_2);
        var result13 = sphere.findIntersections(new Ray(p13, v110)).stream().sorted(Comparator.comparingDouble(p -> p.distance(p13))).toList();
        assertEquals(2, result13.size(), "Wrong number of points");
        assertEquals(exp13, result13, "Ray starts before the sphere");

        // TC14: Ray starts at sphere and goes inside (1 point)
        var result14 = sphere.findIntersections(new Ray(pInside, new Vector(-0.5, -0.5, 0))).stream().sorted(Comparator.comparingDouble(p -> p.distance(pInside))).toList();
        assertEquals(1, result14.size(), "Wrong number of points");
        assertEquals(exp2, result14, "Ray starts at sphere and goes inside");

        // TC15: Ray starts inside (1 point)
        var result15 = sphere.findIntersections(new Ray(pInside, v110)).stream().sorted(Comparator.comparingDouble(p -> p.distance(pInside))).toList();
        assertEquals(1, result15.size(), "Wrong number of points");
        assertEquals(exp2, result15, "Ray starts inside");

        // TC16: Ray starts at the center (1 point)
        var result16 = sphere.findIntersections(new Ray(new Point(0, 0, 0), v110)).stream().sorted(Comparator.comparingDouble(p -> p.distance(new Point(0, 0, 0)))).toList();
